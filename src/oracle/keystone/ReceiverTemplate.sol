...
abstract contract ReceiverTemplate is IReceiver, Ownable {
    ...
    address private s_forwarderAddress; // Only this address can call onReport
    ...
    /// @dev The forwarder address cannot be set to address(0). Without this check, anyone can call
    ///      onReport() with arbitrary data.
    ...
    function setExpectedAuthor(address _author) external onlyOwner {
        if (_author == address(0) && s_expectedWorkflowName != bytes10(0)) {
            revert WorkflowNameRequiresAuthorValidation();
        }
    ...
    ///      You must call setExpectedAuthor() before calling this function.
        ...
        returns (bytes32 workflowId, bytes10 workflowName, address workflowOwner)
    {
        if (metadata.length != METADATA_LENGTH) {
            revert InvalidMetadataLength(metadata.length, METADATA_LENGTH);
        }
