...
4.  **Configure Adapter**:
    ...
    *   **Approve**: The Admin (or Token Owner) must approve the Adapter to spend `PropertyToken`.
        ...
        *   **Crucial**: If the goal is to make the token *available* only via bridging initially, you might lock the supply. If users hold tokens, they approve the adapter individually.
