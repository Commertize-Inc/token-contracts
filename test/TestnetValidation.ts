...
const bridgeContracts = (deployment.bridgeContracts ?? {}) as Record<string, Record<string, string>>;
...
const firstAdapterAddress = firstPropToken ? bridgeContracts[firstPropToken]?.adapter : undefined;

describe(`Testnet Validation — ${networkName} (chainId ${deployment.network?.chainId})`, { timeout: 120_000 }, function () {
	...
	describe("IdentityRegistry", { skip: !contracts.IdentityRegistry }, function () {
		let ir: any;

		beforeEach(async function () {
			ir = await ethers.getContractAt("IdentityRegistry", contracts.IdentityRegistry);
		});
	...
	describe("TokenCompliance", { skip: !contracts.TokenCompliance }, function () {
		let tc: any;

		beforeEach(async function () {
			tc = await ethers.getContractAt("TokenCompliance", contracts.TokenCompliance);
		});
		...
		it("Points to the correct IdentityRegistry", async function () {
			if (!contracts.IdentityRegistry) { this.skip(); return; }
			expect(await tc.identityRegistry()).to.equal(contracts.IdentityRegistry);
		});
	});

	describe("PropertyFactory", { skip: !contracts.PropertyFactory }, function () {
		let factory: any;

		beforeEach(async function () {
			factory = await ethers.getContractAt("PropertyFactory", contracts.PropertyFactory);
		});
	...
	describe("Property Tokens (from Factory)", { skip: !contracts.PropertyFactory }, function () {
		let factory: any;
		...
		it("All factory-deployed tokens have code", async function () {
			if (!hasProperties) { this.skip(); return; }
			for (const addr of propertyAddresses) {
				const code = await deployer.provider.getCode(addr);
				expect(code).to.not.equal("0x", `PropertyToken at ${addr} has no code`);
			}
		});

		it("First property token has valid metadata", async function () {
			if (!hasProperties) { this.skip(); return; }
			const pt = await ethers.getContractAt("PropertyToken", propertyAddresses[0]);
		...
		it("First property token points to correct compliance", async function () {
			if (!hasProperties || !contracts.TokenCompliance) { this.skip(); return; }
			const pt = await ethers.getContractAt("PropertyToken", propertyAddresses[0]);
	...
	describe("Bridge: PropertyTokenAdapter (Home)", { skip: !isHomeChain || !hasBridge || !firstAdapterAddress }, function () {
		let adapter: any;

		beforeEach(async function () {
			adapter = await ethers.getContractAt("PropertyTokenAdapter", firstAdapterAddress!);
		});
		...
		it("Points to correct compliance", async function () {
			if (!contracts.TokenCompliance) { this.skip(); return; }
		...
		it("Is exempt in TokenCompliance", async function () {
			if (!contracts.TokenCompliance) { this.skip(); return; }
			const tc = await ethers.getContractAt("TokenCompliance", contracts.TokenCompliance);
	...
	describe("Bridge: PropertyTokenOFT (Destination)", { skip: isHomeChain || !contracts.PropertyTokenOFT }, function () {
		let oft: any;
		...
		it("Daily bridge cap is configured or unlimited", async function () {
			...
			console.log(`      Daily bridge cap: ${cap === 0n ? "unlimited" : ethers.formatEther(cap)}`);
		});
	});

	// ─── Destination Compliance Infrastructure ────────────────

	describe("Destination Compliance (base-sepolia)", { skip: isHomeChain }, function () {
		it("IdentityRegistry: deployer has admin role", async function () {
			if (!contracts.IdentityRegistry) { this.skip(); return; }
			const ir = await ethers.getContractAt("IdentityRegistry", contracts.IdentityRegistry);
			const adminRole = await ir.DEFAULT_ADMIN_ROLE();
			expect(await ir.hasRole(adminRole, deployer.address)).to.be.true;
		});
		...
		it("TokenCompliance: linked to destination IdentityRegistry", async function () {
			if (!contracts.TokenCompliance || !contracts.IdentityRegistry) { this.skip(); return; }
			const tc = await ethers.getContractAt("TokenCompliance", contracts.TokenCompliance);
			expect(await tc.identityRegistry()).to.equal(contracts.IdentityRegistry);
		});
	});
	...
	describe("LayerZero Configuration", { skip: !meta.LZ_ENDPOINT }, function () {
		...
		it("Deployment JSON LZ config matches network-meta (if present)", async function () {
			if (!deployment.layerZero) {
				console.log("      (no layerZero field in deployment JSON — will be set on next deploy)");
				this.skip();
				return;
			}
	...
	describe("Cross-Chain Bridge Quote", { skip: !hasBridge && !contracts.PropertyTokenOFT }, function () {
		it("Can quote a bridge fee from adapter (home chain)", async function () {
			...
			try {
				...
				console.log(`      Native fee: ${ethers.formatEther(nativeFee)} ${meta.currency}`);
				console.log(`      LZ token fee: ${ethers.formatEther(lzTokenFee)}`);
				expect(nativeFee).to.be.greaterThan(0);
			} catch (err: any) {
				if (err.message.includes("NoPeer") || err.message.includes("peer")) {
	...
	after(function () {
		console.log("\n    ─── Deployment Summary ───");
		console.log(`    Network:    ${networkName} (chainId ${deployment.network?.chainId})`);
