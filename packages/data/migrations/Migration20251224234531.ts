...
export class Migration20251224234531 extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`create table "legal_document" ("id" varchar(255) not null, "related_entity_id" uuid not null, "related_entity_type" text check ("related_entity_type" in ('USER', 'PROPERTY')) not null, "document_type" text check ("document_type" in ('TAX_FORM', 'OM', 'SUBSCRIPTION_AGREEMENT', 'ACCREDITATION_PROOF')) not null, "file_url" varchar(255) not null, "updated_at" date not null, constraint "legal_document_pkey" primary key ("id"));`);
    this.addSql(`create index "legal_document_related_entity_id_index" on "legal_document" ("related_entity_id");`);
    this.addSql(`create index "legal_document_related_entity_type_index" on "legal_document" ("related_entity_type");`);
    ...
    this.addSql(`create table "review_comment" ("id" varchar(255) not null, "entity_type" text check ("entity_type" in ('KYC', 'INVESTOR', 'SPONSOR', 'LISTING')) not null, "entity_id" varchar(255) not null, "author_id" varchar(255) not null, "content" text not null, "is_internal" boolean not null default false, "created_at" date not null, constraint "review_comment_pkey" primary key ("id"));`);
    ...
    this.addSql(`create table "notification" ("id" varchar(255) not null, "user_id" varchar(255) not null, "title" varchar(255) not null, "message" text not null, "type" varchar(255) not null default 'info', "is_read" boolean not null default false, "link" varchar(255) null, "created_at" date not null, constraint "notification_pkey" primary key ("id"));`);

    this.addSql(`create table "investment" ("id" varchar(255) not null, "user_id" varchar(255) not null, "property_id" varchar(255) not null, "amount_usdc" numeric(20,6) not null, "token_count" int not null, "status" text check ("status" in ('PENDING', 'COMPLETED', 'FAILED')) not null default 'PENDING', "transaction_hash" varchar(255) null, "signed_subscription_agreement_url" varchar(255) null, "agreed_to_terms_at" date null, "created_at" date not null, "updated_at" date not null, constraint "investment_pkey" primary key ("id"));`);
    this.addSql(`create index "investment_status_index" on "investment" ("status");`);
    ...
    this.addSql(`alter table "listing" add constraint "listing_sponsor_id_foreign" foreign key ("sponsor_id") references "sponsor" ("id") on update cascade;`);

    this.addSql(`alter table "dividend" add constraint "dividend_listing_id_foreign" foreign key ("listing_id") references "listing" ("id") on update cascade;`);

    this.addSql(`alter table "user" add constraint "user_sponsor_id_foreign" foreign key ("sponsor_id") references "sponsor" ("id") on update cascade on delete set null;`);

    this.addSql(`alter table "review_comment" add constraint "review_comment_author_id_foreign" foreign key ("author_id") references "user" ("id") on update cascade;`);
    ...
    this.addSql(`alter table "investment" add constraint "investment_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade;`);
    this.addSql(`alter table "investment" add constraint "investment_property_id_foreign" foreign key ("property_id") references "listing" ("id") on update cascade;`);
  ...
  override async down(): Promise<void> {
    this.addSql(`alter table "listing" drop constraint "listing_sponsor_id_foreign";`);

    this.addSql(`alter table "user" drop constraint "user_sponsor_id_foreign";`);

    this.addSql(`alter table "dividend" drop constraint "dividend_listing_id_foreign";`);

    this.addSql(`alter table "investment" drop constraint "investment_property_id_foreign";`);

    this.addSql(`alter table "review_comment" drop constraint "review_comment_author_id_foreign";`);
    ...
    this.addSql(`alter table "investment" drop constraint "investment_user_id_foreign";`);
