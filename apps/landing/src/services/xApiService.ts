...
class XApiService {
	...
	async postTweet(data: XPostData): Promise<TweetResponse> {
		...
		if (this.dailyPostCount >= this.RATE_LIMITS.DAILY_POSTS) {
			throw new Error(
				`Daily posting limit of ${this.RATE_LIMITS.DAILY_POSTS} reached`
			);
		}
		...
		try {
			...
			console.log(
				`Tweet posted: ${tweet.data.id} (${this.dailyPostCount}/${this.RATE_LIMITS.DAILY_POSTS} daily)`
			);
			return tweet;
		} catch (error) {
	...
	async getAccountMetrics(): Promise<AccountMetrics | null> {
...
export type { TweetResponse, AccountMetrics, RateLimitStatus };
