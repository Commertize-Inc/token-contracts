...
class XScheduler {
  ...
  start() {
    ...
    if (!xApiService.isReady()) {
      console.log('X API not ready - scheduler will start when credentials are available');
      return;
    }
  ...
  schedulePost(text: string, scheduledTime: Date, media?: string[]): string {
    ...
    console.log(`Post scheduled for ${scheduledTime.toISOString()}: ${text.substring(0, 50)}...`);
    return id;
  }

  cancelPost(postId: string): boolean {
    const index = this.scheduledPosts.findIndex(p => p.id === postId && !p.posted);
    if (index !== -1) {
  ...
  async postNow(text: string, media?: string[]): Promise<any> {
    return xApiService.postTweet({ text, media });
  }

  getStatus(): { isRunning: boolean; pendingPosts: number; rateLimits: any } {
    return {
