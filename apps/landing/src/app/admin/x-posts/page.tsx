...
export default function XPostsAdmin() {
	...
	useEffect(() => {
		// Fetch status on mount and set up polling - legitimate data fetching pattern
	...
	const handleForcePost = async () => {
		if (
			!confirm(
				"Are you sure you want to post now? This will create a live post on X."
			)
		) {
			return;
		}
		...
		try {
			...
			if (data.success) {
				setMessage({
					type: "success",
					text: `Successfully posted! Tweet ID: ${data.tweetId}`,
				});
				fetchStatus();
			} else {
		...
		<div className="min-h-screen bg-gray-50 py-8 px-4">
			<div className="max-w-4xl mx-auto">
				...
				<p className="text-gray-600 mb-8">
					Manage AI-generated X posts for Commertize
				</p>
						...
						{status ? (
							<div className="space-y-3">
								<div className="flex justify-between">
									...
									<span
										className={`font-medium ${status.apiReady ? "text-green-600" : "text-red-600"}`}
									>
								...
								<div className="flex justify-between">
									...
									<span className="font-medium text-green-600">
										{status.successfulPosts}
									</span>
								</div>
								<div className="flex justify-between">
									<span className="text-gray-600">Today</span>
									<span
										className={`font-medium ${status.isWeekday ? "text-green-600" : "text-yellow-600"}`}
									>
										{status.isWeekday
											? "Weekday (Posts Enabled)"
											: "Weekend (Posts Disabled)"}
									</span>
								</div>
							</div>
						) : (
						...
						<div className="space-y-4">
							...
							<div>
								...
								<p className="text-sm text-gray-500 mt-1">
									{preview.text.length}/280 characters
								</p>
							</div>
							<div>
								<label className="block text-sm font-medium text-gray-600 mb-1">
					...
					{history.length > 0 ? (
						<div className="space-y-4">
							{history.map((post, index) => (
								<div
									key={index}
									className={`p-4 rounded-lg border ${post.success ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}
								>
									<div className="flex justify-between items-start mb-2">
										<span
											className={`text-sm font-medium ${post.success ? "text-green-700" : "text-red-700"}`}
										>
				...
				<div className="mt-8 bg-gray-100 rounded-xl p-6">
					...
					<div className="text-sm text-gray-600 space-y-2">
						...
						<p className="mt-4">
							Topics covered: Real estate tokenization, RWA, AI-powered
							investing, Commertize platform updates, and commercial real estate
							innovation.
						</p>
					</div>
				</div>
