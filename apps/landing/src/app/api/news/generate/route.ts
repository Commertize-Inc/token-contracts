...
const NEWS_CATEGORIES = [
  ...
  { id: 'Tokenization', name: 'Tokenization', description: 'Asset tokenization trends & developments' },
  { id: 'RWA', name: 'RWA', description: 'Real World Assets on blockchain' },
  { id: 'Crypto', name: 'Crypto', description: 'Cryptocurrency market updates' },
  { id: 'DeFi', name: 'DeFi', description: 'Decentralized Finance news' },
  { id: 'CREUSD', name: 'CREUSD', description: 'Commertize stablecoin updates' },
  { id: 'Digital Assets', name: 'Digital Assets', description: 'Digital asset ecosystem news' },
  { id: 'Regulation', name: 'Regulation', description: 'SEC, legal, compliance updates' },
  { id: 'Technology', name: 'Technology', description: 'PropTech & blockchain innovation' },
  { id: 'Markets', name: 'Markets', description: 'Financial markets & investment trends' },
  { id: 'Datacenters', name: 'Datacenters', description: 'Data center real estate & infrastructure' },
  { id: 'Green Energy Infrastructure', name: 'Green Energy Infrastructure', description: 'Renewable energy investments' },
  { id: 'Solar Farms', name: 'Solar Farms', description: 'Solar energy asset tokenization' },
  { id: 'Wind Farms', name: 'Wind Farms', description: 'Wind energy infrastructure' },
  { id: 'OmniGrid', name: 'OmniGrid', description: 'Green infrastructure network' },
];

async function getRecentCategories(pool: ReturnType<typeof getPool>, limit: number = 3): Promise<string[]> {
  try {
...
async function recordCategoryUsage(pool: ReturnType<typeof getPool>, category: string): Promise<void> {
  try {
    await pool.query(
...
function selectCategory(recentCategories: string[]): typeof NEWS_CATEGORIES[0] {
...
async function saveArticle(pool: ReturnType<typeof getPool>, article: {
  id: string;
  slug: string;
  title: string;
  summary: string;
...
export async function POST(request: NextRequest) {
  try {
    const { count = 1, saveToDb = true } = await request.json().catch(() => ({ count: 1, saveToDb: true }));
...
Focus on current trends, market insights, and actionable information for investors interested in tokenized real estate and sustainable infrastructure. Make the content timely and relevant.`
      ...
      try {
        imageUrl = await generateArticleImage(articleData.title, selectedCategory.name);
      } catch (imgError) {
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    usage: 'POST with optional { count: number, saveToDb: boolean } to generate articles',
    totalCategories: NEWS_CATEGORIES.length,
    categories: NEWS_CATEGORIES.map(c => ({ id: c.id, description: c.description })),
