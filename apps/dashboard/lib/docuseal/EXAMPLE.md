...
export async function POST(req: NextRequest) {
  try {
    const { userId, email, name, investmentAmount, propertyName } = await req.json();
...
import { NextRequest, NextResponse } from 'next/server';
import { getSubmission, formatSubmissionStatus, getSubmissionProgress } from '@/lib/docuseal';
    ...
    switch (event_type) {
      case 'form.completed':
        ...
        if (data.metadata?.userId) {
          ...
          const user = await em.findOne(User, { id: data.metadata.userId as string });
          if (user) {
