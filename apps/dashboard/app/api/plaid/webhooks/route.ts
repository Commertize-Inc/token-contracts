	...
	try {
		...
		switch (event.webhook_code) {
			...
			case 'WEBHOOK_UPDATE_ACKNOWLEDGED':
				...
				console.log('[Plaid Webhook] Webhook URL updated for item:', plaidItem.id);
				break;

			case 'DEFAULT_UPDATE':
				// General item update - set back to active if no errors
				if (plaidItem.status === 'login_required' || plaidItem.status === 'error') {
			...
			case 'TRANSACTIONS_REMOVED':
				...
				console.log('[Plaid Webhook] Transactions removed for item:', plaidItem.id);
				break;
